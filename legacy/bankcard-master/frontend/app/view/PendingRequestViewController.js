Ext.define("bcp.view.PendingRequestViewController", {
    ...
    onEditResubmit: function (button, e, eOpts) {
        ...
        if (rec.data.statusCode === 11 || rec.data.statusCode === 14) {
            ...
            Ext.Msg.alert("Warning", "Someone is working on the request and you cannot edit it.");
        }
    },
    ...
    onSelect: function (rowmodel, record, index, eOpts) {
        ...
        //make sure aciton buttons are disabled first
        ...
        if (status === 11 || status == 14) {
            ...
            refs.btnResend.enable();
        } else if (status == 7 && bcp.util.CommonUtil.isUserInRole(["Bankcard Holder"])) {
            // Allow BCH to edit when status is 7
            refs.btnEdit.enable();
        }
        ...
        if (component.requestId && component.requestId !== 0) {
            ...
            if (rec) {
                ...
                if (detail.items.length === 0) {
                    ...
                    if (
                        (type == 12 && (!pref || pref.get("justPrefVal") == "O" || pref.get("justPrefVal") == "Y")) ||
                        type != 12
                    ) {
                        detail.add({
                    ...
                    if (
                        (type == 12 &&
                            (!pref || pref.get("financePrefVal") == "O" || pref.get("financePrefVal") == "Y")) ||
                        type != 12
                    ) {
                        detail.add({
                            xtype: "financepanel",
                    ...
                    if (routeController.isBaoReviewing(rec)) {
                        if (pref.get("upToPrefVal") == "Y" && rec.get("approvalAmount") == 0) {
                            rec.set("approvalAmount", pref.get("upToPrefValDetail") + rec.get("totalCost"));
                        }
                    }
            ...
            if (jform.isVisible() && jSaveBtn && jSaveBtn.isVisible()) {
                if (jform.isValid()) {
                    ...
                    if (vals.isCv !== undefined) {
                        Ext.Msg.alert("Validation Error", "Please fix the invalid data in the Justification Form");
                        return false;
                    } else {
