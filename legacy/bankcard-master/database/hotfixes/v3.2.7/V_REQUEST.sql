DROP VIEW BCPMS_OWNER.V_REQUEST;
...
(REQUEST_ID, FY, NOTES, REQUESTER_ID, REQUESTER_NAME, 
 ...
 GRP_ORG_ID, IS_IT_PURCHASE, ITSO_APPROVED)
BEQUEATH DEFINER
AS 
SELECT req.request_id,
       ...
       WHERE     req.request_id = r.request_id
             AND req.request_id = rv.request_id(+) --request may not have a vendor yet
             AND req.request_id = vri.request_id(+) --request may not have a item yet
             AND req.request_id = vris.request_id(+) --request may not have a item status yet
