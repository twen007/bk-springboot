...
IS
    /**
    2/6/23 fixed some code that update org data in this proc after we introduced detailee mode
	6/13/23 updated to insert default div preference record if not exists
    **/
...
BEGIN
    ...
    --find who is the official requester (added 2/6/23, add the group id needed for detailee mode modification below)
    SELECT requester_id, grp_id
      INTO v_requester_id, v_grp_id
      FROM request
     WHERE request_id = p_request_id;
    ...
    --detailee mode: commented out code above to get org data from requester.since the detailee mode can allow a user with proper privilege to create
    --request for a different org and theere's a trigger to insert proper org data for request, we should get org data from the request record itself
    SELECT SUBSTR (b.ORG_CD, 0, 3),
           b.ORG_CD,
           b.OU_ORG_ID,
           b.DIV_ORG_ID
      INTO v_pc_div,
           ...
           v_div_id
      FROM nist_group b
     WHERE b.ORG_ID = v_grp_id;

    --check if division preference was set
    SELECT COUNT (*)
      INTO v_div_pref_ct
      FROM div_preferences
     WHERE div_id = v_div_id;

    --if div preference is not set yet, insert a default record for the division
    IF v_div_pref_ct = 0
    THEN
        INSERT INTO DIV_PREFERENCES (DIV_ID,
                                     JUST_PREF_VAL,
                                     FINANCE_PREF_VAL,
                                     SHIPPING_COST_PREF_VAL,
                                     SHIPPING_COST_PREF_VAL_DETAIL,
                                     UP_TO_PREF_VAL,
                                     UP_TO_PREF_VAL_DETAIL)
             VALUES (v_div_id,
                     'O',
                     'O',
                     'R',
    ...
    SELECT just_pref_val, finance_pref_val
      INTO l_justification_preference, l_finance_preference
      FROM div_preferences
     WHERE div_id = v_div_id;
    ...
    ELSE
        ...
        LOOP
            ...
            THEN
                v_has_taggable_equipment :=
                    v_item (indx2).IS_TAGGABLE_EQUIPMENT;
            END IF;
  ...
  **/
        ...
        --after detailee mode is added and with the org data insert trigger, there's no need to update org data here
        ...
        IF v_has_chemical = 'Y'
        THEN
            --detailee mode change, the code below to get group cd by request id and requester id is no longer valid
            --we should just use the grp_id of the request to get group cd
            --SELECT g.org_cd
            --  INTO v_codes
            --  FROM request  r
            --       INNER JOIN nist_user u ON r.requester_id = u.people_id
