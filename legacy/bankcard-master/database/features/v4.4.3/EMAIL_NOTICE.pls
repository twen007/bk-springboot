...
DECLARE
    ...
    */
    ...
    l_omit_notification      INTEGER := :new.omit_notification;
    l_route_type_name        VARCHAR2 (60) := '';
    l_route_comment          NVARCHAR2 (1000) := :new.route_notes;
...
BEGIN
    ...
    THEN
        ...
        l_always_valid_link :=
               'Please be noted that the request moves within the approval process over time and the link above may not always work. To <b>view this request at any time</b>, click this <a href="'
            || l_app_url
        ...
        --issue 718
        IF l_route_comment is not NULL
        THEN
            l_route_comment := '<br><br><b>Comment:</b> ' || l_route_comment || '<br><br>';
        ELSE
            l_route_comment := '';
        END IF; 
        ...
        -- move comment in between link messages in the body, see issue 718
       ...
       END IF;**/
        ...
        --5/27/25 changed from emails to ids so we can query this column for backup ITSOs
        if l_also_notify is not null and LENGTH(TRIM(l_also_notify)) > 0
        then
            --call function to translate ids into emails
            l_also_notify := get_emp_emails(l_also_notify);
        end if;
        ...
        THEN
            ...
            l_supervisor_email := l_app_bcc;
            l_receip2_email := l_app_bcc;
            l_subject := 'TEST TEST TEST !!!' || l_also_notify || l_subject;
            if l_also_notify is not null
            then
                l_also_notify := l_app_bcc;
            end if;
        ...
        THEN
            ...
            IF l_route_type_id <> 7 AND NOT l_omit_notification = 1
            THEN                                             -- 7 is received. omit_notification indicate the route does not need to send any notification out
                up_send_email (
                    l_sender_email,
                    l_receip_email,
                    l_cc_email,
        ...
        THEN
            up_send_email (
                ...
                || ' has been ' || l_route_type_name,
                 l_body_notify
                || '<br>'
                || 'To <b>view this request at any time</b>, click this <a href="'
                || l_app_url
                || '?subview=requestsearching/' --'#requestsearching/'
                || :new.request_id
                || '">link</a> to see the detail.</b>'
                || l_foot);
        END IF;
