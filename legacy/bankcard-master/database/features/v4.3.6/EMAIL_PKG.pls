create or replace PACKAGE BODY             EMAIL_PKG AS
    ...
    */
    ...
    s_msg_notify_approver           VARCHAR2 (3000)     := 'This is a notification that a bankcard purchase request $1 has a need by date on $2 but the request is still not approved. Please review the request in your [Pending Actions].';
...
procedure NOTIFY_BCH_OF_UP_ORDER
    ...
    begin
        ...
        declare cursor cur_route is
        select r.REQUEST_ID, r.route_to, r.route_to_name, BANKCARD_HOLDER_ID, to_char (NEEDED_BY_DATE,'mm/dd/yyyy') as nbd
        from 
        ...
        begin
            for rec in cur_route loop
                ...
                if not is_prod then
                    ...
                    --send notification to approvers too                  
                    --g_msg := g_header || get_user_email (l_to_id_approver) || rec.route_to_name || ':<br><br>' || s_msg_notify_approver || g_footer;
                    --g_msg := replace (g_msg, '$1', l_request_id || ' (' || g_app_url || l_request_id || ')');
                    --g_msg := replace (g_msg, '$2', l_nbd);
                ...
                if is_prod then 
                    ...
                    --send notification to approvers too
                    g_to := get_user_email (l_to_id_approver);
                    g_to_name := rec.route_to_name;
                    g_bcc := 'xinweiw@nist.gov';
                    g_msg := g_header || g_to_name || ':<br><br>' || s_msg_notify_approver || g_footer;
                    g_msg := replace (g_msg, '$1', l_request_id || ' (' || g_app_url || l_request_id || ')');
                    g_msg := replace (g_msg, '$2', l_nbd);
                    send_email;
                end if;
